...
# Browser-like headers to prevent server from blocking bot requests
API_HEADERS = {
...
def fetch_history(game_type, count=100):
    ...
    try:
        r = requests.get(url, params={"pageNo": 1, "pageSize": count}, headers=API_HEADERS, timeout=10)
        raw = r.text.strip() if r.content else ""
        if not raw:
            logging.error("fetch_history: empty response from API")
            return []
        if raw.startswith("<"):
            logging.error(f"fetch_history: got HTML instead of JSON — API may be blocking requests")
            return []
        try:
            data = r.json()
...
def check_period_result(game_type, period_id, retries=10):
    ...
    for attempt in range(retries):
        time.sleep(4)
        try:
            r = requests.get(url, params={"pageNo": 1, "pageSize": 20}, headers=API_HEADERS, timeout=10)
            raw = r.text.strip() if r.content else ""
            if not raw:
                logging.warning(f"check_period_result attempt {attempt+1}: empty response, retrying...")
                continue
            if raw.startswith("<"):
                logging.warning(f"check_period_result attempt {attempt+1}: got HTML instead of JSON (API blocked/down), retrying...")
                time.sleep(3)
                continue
            try:
                data = r.json()
